...
export class ThumbScheduler {
  private jobs = new Map<string, Job>(); // key -> latest job
  private stack: string[] = []; // request order; newest at the end
  private active = 0;

  constructor(private concurrency = 6) {}

  request(key: string, path: string, max: number, cb: (url: string) => void): void {
